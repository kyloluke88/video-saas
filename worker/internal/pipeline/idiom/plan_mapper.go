...
func toServiceProjectPlan(plan dto.ProjectPlanResult) services.ProjectPlanResult {
	s := make([]services.ScenePlan, 0, len(plan.Scenes))
	...
	registryItems := make([]dto.ObjectSpec, 0, len(plan.ObjectRegistry.Characters)+len(plan.ObjectRegistry.Props)+len(plan.ObjectRegistry.Environments))
	...
	registry := make([]services.ObjectSpec, 0, len(registryItems))
	for _, item := range registryItems {
		...
		registry = append(registry, services.ObjectSpec{
			ID:        item.ID,
			Type:      itemType,
			Label:     item.ID,
	...
	return services.ProjectPlanResult{
		ProjectID:         "",
		Platform:          "both",
		...
		VisualBible: services.VisualBible{
...
func extractObjectIDs(items []dto.ObjectSpec) []string {
...
func containsObject(items []dto.ObjectSpec, id string) bool {
	for _, item := range items {
		if item.ID == id {
			return true
		}
