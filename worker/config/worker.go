...
func init() {
	config.Add("worker", func() map[string]interface{} {
		return map[string]interface{}{
			"worker_role":                config.Env("WORKER_ROLE", "all"),
			"rabbitmq_split_queues":      cast.ToBool(config.Env("RABBITMQ_SPLIT_QUEUES", false)),
			...
			"rabbitmq_align_queue":       config.Env("RABBITMQ_ALIGN_QUEUE", "video.tasks.align"),
			"rabbitmq_align_routing_key": config.Env("RABBITMQ_ALIGN_ROUTING_KEY", "video.align"),
			"rabbitmq_align_retry_queue": config.Env("RABBITMQ_ALIGN_RETRY_QUEUE", "video.tasks.align.retry"),
			"rabbitmq_align_retry_routing_key": config.Env(
				"RABBITMQ_ALIGN_RETRY_ROUTING_KEY",
				"video.align.retry",
			),
			"rabbitmq_align_dlq":             config.Env("RABBITMQ_ALIGN_DLQ", "video.tasks.align.dlq"),
			"rabbitmq_align_dlq_routing_key": config.Env("RABBITMQ_ALIGN_DLQ_ROUTING_KEY", "video.align.dlq"),
			...
			"podcast_tts_tempo": cast.ToFloat64(
				config.Env("PODCAST_TTS_TEMPO", config.Env("GOOGLE_TTS_SPEAKING_RATE", 1.0)),
			),
			"podcast_tts_ja_tempo": cast.ToFloat64(
				config.Env("PODCAST_TTS_JA_TEMPO", config.Env("GOOGLE_TTS_JA_SPEAKING_RATE", 0.88)),
			),
			...
			"google_tts_practical_male_voice_ids": config.Env(
				"GOOGLE_TTS_PRACTICAL_MALE_VOICE_IDS",
				"Orus,Achird,Charon,Rasalgethi",
			),
			"google_tts_practical_female_voice_id": config.Env(
				"GOOGLE_TTS_PRACTICAL_FEMALE_VOICE_ID",
				"Leda",
			),
			"google_tts_practical_female_voice_ids": config.Env(
				"GOOGLE_TTS_PRACTICAL_FEMALE_VOICE_IDS",
				"Achernar,Kore,Erinome,Despina",
			),
			...
			"elevenlabs_tts_male_voice_id":    config.Env("ELEVENLABS_TTS_MALE_VOICE_ID", "iLVmqjzCGGvqtMCk6vVQ"),
			"elevenlabs_tts_male_voice_ids":   config.Env("ELEVENLABS_TTS_MALE_VOICE_IDS", "Uyx98Ek4uMNmWN7E28CD,lnIpQcZuikKim3oNdYlP,zCgijgIKIMkFHnzXcCva,NO5A3b3sSzDyJQF7MiNS"),
			"elevenlabs_tts_female_voice_id":  config.Env("ELEVENLABS_TTS_FEMALE_VOICE_ID", "RBnMinrYKeccY3vaUxlZ"),
			"elevenlabs_tts_female_voice_ids": config.Env("ELEVENLABS_TTS_FEMALE_VOICE_IDS", "GxhGYQesaQaYKePCZDEC,MXKtCrra8fvlDUbfKUT1,a0MsDWokG5Xsuji8g8er,MNKK2Wl2wbbsEPQTHZGt"),
			"elevenlabs_tts_narrator_voice_id": config.Env(
				"ELEVENLABS_TTS_NARRATOR_VOICE_ID",
				"",
			),
