...
func Persist(input PersistInput) (PersistResult, error) {
	upsert, err := BuildPageUpsert(input)
	if err != nil {
	...
	pageID, err := store.UpsertPodcastScriptPage(upsert)
	if err != nil {
		return PersistResult{}, err
	}
...
func BuildPageUpsert(input PersistInput) (persistence.ScriptPageUpsert, error) {
	...
	return BuildPageUpsertFromProjectDir(projectDir, input)
}

func BuildPageUpsertFromProjectDir(projectDir string, input PersistInput) (persistence.ScriptPageUpsert, error) {
	if strings.TrimSpace(input.ProjectID) == "" {
		return persistence.ScriptPageUpsert{}, fmt.Errorf("project_id is required")
	}
	...
	return persistence.ScriptPageUpsert{
...
func buildSectionBody(blocks []dto.PodcastBlock) string {
...
func buildLine(language string, seg dto.PodcastSegment) lineDocument {
	...
	return lineDocument{
		...
		SpeakerName: speakerDisplayName(language, seg.Speaker),
		Text:        text,
...
func buildPageSlug(script dto.PodcastScript, projectID string) string {
	base := slugify(coalesce(script.YouTube.PublishTitle, script.Title))
	if base == "" {
		return projectID
	}
	suffix := slugify(projectID)
	if suffix == "" {
		return base
	}
	return base + "-" + suffix
...
func speakerDisplayName(language string, speaker string) string {
	switch normalizeSpeaker(speaker) {
	case "female":
		if isJapaneseLanguage(language) {
			return "Yui"
		}
...
func normalizeSpeaker(value string) string {
	...
	switch normalized {
	case "female", "f", "woman", "girl", "panpan", "盼盼", "女":
		return "female"
	default:
		return "male"
	}
