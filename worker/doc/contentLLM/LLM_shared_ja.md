...
- 当 target_duration_minutes >= 15 时，整集里 male 与 female 都至少要各自出现一次同 speaker 连续多个 segment 的情况
- 当 target_duration_minutes >= 15 时，male 的连读次数必须大于等于 female 的连读次数
...
- 5 分钟内容：建议 36 到 46 个 segments，全部 display_ja 正文总量约 1800 到 2400 字符
- 10 分钟内容：建议 72 到 92 个 segments，全部 display_ja 正文总量约 3600 到 4800 字符
- 15 分钟内容：建议 108 到 138 个 segments，全部 display_ja 正文总量约 5400 到 7200 字符
- 20 分钟内容：建议 144 到 184 个 segments，全部 display_ja 正文总量约 7200 到 9600 字符
...
- tts_ja 去掉标签后也必须满足 min_chars_per_segment 和 max_chars_per_segment
...
2. 是否已逐条自检所有 segment，确认每条 display_ja 都满足 min_chars_per_segment 和 max_chars_per_segment 的范围要求，且对应结构完整正确
...
5. 最后一个 block 是否为 summary_cta，且最后一个 segment 是否为唯一的 summary=true 收尾句
6. 最后一个 segment 是否明确包含本集 topic 的简短回顾，以及鼓励继续练习日语的表达
7. 所有 display_ja 是否满足标点清洁规则
8. 全部内容是否符合当前输入规则中的规模、风险控制和风格要求
      ...
      "segments": [
        {
          "segment_id": "seg_001",
          "speaker": "female",
          "display_ja": "みなさん、こんにちは。",
          "tts_ja": "[cheerfully] みなさん、こんにちは。",
          "en": "Hi everyone.",
          "summary": false,
          "ruby_tokens": [
