...
7. 允许少量自然的情绪反应，例如轻笑、轻微叹气、惊讶、犹豫、柔和的吐槽、会心一笑、顺势接话、轻轻重复对方的关键词。注意区分：
- text 是中文字幕台词，必须干净，不允许写标签（例如「[笑]」「（笑）」）
- 当 tts_type = google：情绪写在自然中文口语里（例如“哈哈”“呵呵”“哎呀”“诶？”“嗯…”），speech_text 设为 ""
- 当 tts_type = eleven：text 仍保持干净；可在 speech_text 中使用白名单标签表达语气/情绪
...
- 10 分钟内容：建议 5 到 6 个 chapter、7 到 9 个 block、55 到 70 个 segments
- 15 分钟内容：建议 6 到 7 个 chapter、9 到 12 个 block、80 到 90 个 segments
- 20 分钟内容：建议 7 到 9 个 chapter、10 到 12 个 block、95 到 115 个 segments
...
- 当 tts_type = eleven：
  - 默认 speech_text = text
  - 仅在需要额外情绪/语气控制时，才在 speech_text 中插入白名单标签
  - 标签格式必须为英文方括号：`[tag]`
  - 每个 segment 最多使用 2 个标签（建议 0~1 个）
  - 禁止连续堆叠多个标签，禁止每句都加标签
  - 不允许自造标签；如果不确定，宁可不用标签
  - 标签位置规则：
    - [happy]/[excited]/[sad]/[thoughtful]/[curious]/[surprised]/[cheerfully] 优先放句首
    - [laughing]/[chuckles]/[sigh]/[sighs] 可放句尾（尾部反应）或句中（更自然）
